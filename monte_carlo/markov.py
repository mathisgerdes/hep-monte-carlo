...
class AbstractMarkovUpdate(object):
    ...
    def init_sampler(self, initial, out_mask=None):
        ...
        if len(initial) != self.dim:
            raise ValueError("initial must be of dimension self.dim=" +
                             str(self.dim))
        self.state = initial
...
class MetropolisUpdate(AbstractMetropolisUpdate):
    ...
    def __init__(self, dim, pdf, proposal=None, proposal_pdf=None):
...
class CompositeMetropolisUpdate(AbstractMarkovUpdate):

    def __init__(self, dim, updates, masks=None):
...
class MixingMetropolisUpdate(AbstractMarkovUpdate):

    def __init__(self, dim, updates, weights=None, masks=None):
