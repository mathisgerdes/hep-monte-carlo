...
class SphericalNUTS(DualAveragingSphericalHMC):
    ...
    def __init__(self, ndim, target_log_pdf, target_log_pdf_gradient, start, adapt_schedule, lim_lower=None, lim_upper=None, t0=10, stepsize_bar0=1, Hbar0=0, gamma=0.05, kappa=0.75, delta=0.65, Emax=1000):
        super().__init__(ndim, target_log_pdf, target_log_pdf_gradient, 1., start, adapt_schedule, lim_lower, lim_upper, t0, stepsize_bar0, Hbar0, gamma, kappa, delta)
        self.Emax = Emax
        self.alpha = None
        self.n_alpha = None
        
    def proposal(self, current_q):
        z0 = np.random.standard_normal(self.ndim)
        u = np.random.uniform()
        q_minus, q_plus, z_minus, z_plus, q = current_q, current_q, z0, z0, current_q
        j, n, s = 0, 1, 1

        while s == 1:
            v = np.random.choice([-1, 1])
            if v == -1:
                q_minus, z_minus, _, _, q_prime, n_prime, s_prime, self.alpha, self.n_alpha = self.buildtree(q_minus, z_minus, u, v, j, self.stepsize, q, z0, self.Emax)
            else:
                _, _, q_plus, z_plus, q_prime, n_prime, s_prime, self.alpha, self.n_alpha = self.buildtree(q_plus, z_plus, u, v, j, self.stepsize, q, z0, self.Emax)
    ...
    def buildtree(self, q, z, u, v, j, stepsize, q0, z0, Emax):
        if j == 0:
            # Base case - take one leapfrog step in the direction v.
            q_prime, z_prime = integrator(q, self.target_log_pdf_gradient, z, self.theta_to_x, self.J_xtheta, v*stepsize, nsteps=1)
    
            E = -self.target_log_pdf(self.theta_to_x(q_prime)) - .5*np.sum(z_prime**2)
            E0 = -self.target_log_pdf(self.theta_to_x(q0)) - .5*np.sum(z0**2)
            dE = E - E0
            
            n_prime = (np.log(u) - dE <= 0)
            s_prime = (np.log(u) - dE < Emax)
            return q_prime, z_prime, q_prime, z_prime, q_prime, n_prime, s_prime, min(1, np.exp(dE)), 1
        else:
            # Recursion - implicitly build the left and right subtrees.
            q_minus, z_minus, q_plus, z_plus, q_prime, n_prime, s_prime, alpha_prime, n_alpha_prime = self.buildtree(q, z, u, v, j-1, stepsize, q0, z0, Emax)
            if s_prime == 1:
                if v == -1:
                    q_minus, z_minus, _, _, q_2prime, n_2prime, s_2prime, alpha_2prime, n_alpha_2prime = self.buildtree(q_minus, z_minus, u, v, j-1, stepsize, q0, z0, Emax)
                else:
                    _, _, q_plus, z_plus, q_2prime, n_2prime, s_2prime, alpha_2prime, n_alpha_2prime = self.buildtree(q_plus, z_plus, u, v, j-1, stepsize, q0, z0, Emax)
                ...
                dq = q_plus - q_minus
                s_prime = s_2prime * (np.dot(dq, z_minus) >= 0) * (np.dot(dq, z_plus) >= 0)
                n_prime = n_prime + n_2prime
            return q_minus, z_minus, q_plus, z_plus, q_prime, n_prime, s_prime, alpha_prime, n_alpha_prime
        
    def step(self, current_q):
        return self.proposal(current_q)
